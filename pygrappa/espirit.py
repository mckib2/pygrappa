...
def _fft(x, s=None, axes=None):
    return np.fft.fftshift(
        np.fft.fftn(np.fft.ifftshift(
            x, axes=axes), s=s, axes=axes), axes=axes)
...
def espirit(kspace, calib, kernel_size=None, eig_thresh_1=0.02, eig_thresh_2=0.95, coil_axis=-1):
    ...
    ax = (0, 1)
    t0 = time()
    k = _fft(np.conj(k[::-1, ::-1, ...])*np.sqrt(sx*sy), s=(sx, sy), axes=ax)/np.sqrt(kx*ky)
...
if __name__ == '__main__':
    ...
    matdata = loadmat('/home/nicholas/Downloads/SPIRiT_v0.3/ESPIRiT/data/brain_8ch.mat')
    data = matdata['DATA']
